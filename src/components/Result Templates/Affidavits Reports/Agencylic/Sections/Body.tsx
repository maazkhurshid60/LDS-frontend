...
const Body: React.FC<BodyProps> = (item) => {
    ...
    return <div className="flex flex-col items-start gap-y-2 text-sm">
        <p>State of New York, County of <span className="mx-6 font-semibold capitalize ">{item?.serverAddress ? item?.serverAddress : "______________________________"}</span> ss:</p>
        <p>I, <span className="mx-6 font-semibold mx-3">{item?.serverName ? item?.serverName : "______________________________"}</span> being duly sworn, deposes and says that deponent is not party to this proceeding, is over 18 years of age and resides in
            &nbsp; <span className="mx-6 font-semibold capitalize">{item?.serverAddress ? item?.serverAddress : "______________________________"}</span>, New York  that on <span className="mx-6 font-semibold">{item?.dateOfService ? formatDate(item?.dateOfService) : "______________________________"}</span> at <span className="mx-6 font-semibold">{item?.timeOfService ? formatTime(item?.timeOfService) : "______________________________"}</span>.</p>
        <p>At the property sought to be recovered at <span className="mx-6 font-semibold">{item?.address ? item?.address : "______________________________"}</span> Apt# <span className="mx-6 font-semibold">{item?.apt ? item?.apt : "______________________________"}</span></p>
        <p>The <span className="mx-6 font-semibold">{item?.serviceTypeLTOrStandard ? item?.serviceTypeLTOrStandard : "______________________________"}</span> <br /> was served on : <span className="mx-6 font-semibold">{item?.firstNames ? item?.firstNames : "______________________________"}</span></p>
        ...
        <div className="flex justify-start gap-x-4">
            ...
            <div>
                <p>Suitable age person: substitute served by delivering thereat a true copy for each respondent personally with <span className="mx-6 font-semibold">{item?.substituteDelivered ? item?.substituteDelivered : "______________________________"} </span>,
        ...
        <div className="flex justify-start gap-x-4">
            ...
            <div>
                <p>Posted on door: by affixing a true copy thereof  for each respondent on conspicuous part to wit; the entrance door of said property
                    , the tenant(s)/ occupant(s) place of bussiness premises is recipients <span className="mx-6 font-semibold">{item?.reciepientTitle ? item?.reciepientTitle : "______________________________"}</span> with in state. Deponent was unable to find
                    respondent(s) or to find a person suitable age and discretion who ______________________________ thereat during either of the two service
                    attempts made on the following dates....
                </p>
                <p>Prior Attempt Made On:<span className="mx-6 font-semibold">{(item?.affidavitName === "personal" || item?.affidavitName === "personalplus" || item?.affidavitName === "substitute") ? "________________________" : item?.firstDateAttempt ? formatDate(item?.firstDateAttempt) : "______________________________"}</span>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; at <span className="mx-6 font-semibold">{(item?.affidavitName === "personal" || item?.affidavitName === "personalplus" || item?.affidavitName === "substitute") ? "________________________" : item?.firstTimeAttempt ? formatTime(item?.firstTimeAttempt) : "______________________________"} </span> </p>
                ...
                <p>Approx, Description: Sex:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "__________________" : item?.sex ? item?.sex : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Skin Color:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "___________________" : item?.skinColor ? item?.skinColor : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    Age:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "__________________" : item?.age ? item?.age : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Height:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "____________________" : item?.height ? item?.height : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    Weight:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "________________" : item?.weight ? item?.weight : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Hair:<span className="mx-6 font-semibold">{item?.affidavitName === "conspicuous" ? "______________________" : item?.hair ? item?.hair : "______________________________"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    other identifying features/comments:<span className="mx-6 font-semibold">{item?.otherFeatures ? item?.otherFeatures : "______________________________"}</span>
                </p>
            </div>
        </div>
        <div className="flex justify-start gap-x-4">
            ...
            <p> State of New York, Country of <span className="mx-6 font-semibold"> {item?.serverAddress ? item?.serverAddress : "______________________________"}</span> ,being duly sworn and says that I am not a party to this action and I am over 18 years of age.
        ...
        <div className=" flex items-center justify-between w-full">
            <div>
                ...
                <p className="font-semibold">{item?.dateOfMailing ? formatDate(item?.dateOfMailing) : "______________________________"}</p>
            </div>
            <div className="font-semibold capitalize border-t-[1.5px] border-t-[#000]">
                <p><span >{item?.serverName ? item?.serverName : "______________________________"}</span></p>
                <p>Lic#{item?.lic ? item?.lic : "______________________________"}</p>
                <p><span  >{item?.serverAddress ? item?.serverAddress : "______________________________"}</span>
                </p>
