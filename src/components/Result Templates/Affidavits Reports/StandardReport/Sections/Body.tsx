...
const Body: React.FC<BodyProps> = (item) => {
    return <div className="flex flex-col gap-y-4">
        ...
        <p>On <span className="font-semibold"> {item?.dateOfService ? formatDate(item?.dateOfService) : "______________________________"}</span> Apt#/Desc: <span className="font-semibold">{item?.apt ? item?.apt : "______________________________"} </span>
        ...
        <div className="flex justify-start gap-x-4"><GrCheckbox size={30} />
            <div>
                ...
                <p>First Attempt Made On: <span className="font-semibold"> {item?.firstAttemptDate ? formatDate(item?.firstAttemptDate) : "______________________________"}</span> at <span className="font-semibold"> {item?.firstAttemptTime ? formatTime(item?.firstAttemptTime) : "______________________________"}</span>  </p>
                <p>Second Attempt Made On:<span className="font-semibold"> {item?.secondAttemptDate ? formatDate(item?.secondAttemptDate) : "______________________________"}</span> at <span className="font-semibold"> {item?.secondAttemptTime ? formatTime(item?.secondAttemptTime) : "______________________________"}</span>   </p>
                <p>Date of Service:<span className="font-semibold"> {item?.dateOfService ? formatDate(item?.dateOfService) : "______________________________"}</span>
