...
const Body: React.FC<BodyProps> = (item) => {
    return <div className="flex flex-col gap-y-4">
        ...
        <p>On <span className="font-semibold"> {item?.dateOfService ? item?.dateOfService : "--N/A--"}</span> Apt#/Desc: <span className="font-semibold">{item?.apt ? item?.apt : "--N/A--"} </span>
        ...
        <p className="flex justify-start gap-x-4"><GrCheckbox size={18} /><span className="font-semibold"></span> _______________________________________________________________</p>
        
        <p className="flex justify-start gap-x-4"><GrCheckbox size={30} /> <span className="font-semibold">Corporation</span> served the within named domestic corporation by delivering a true copy of the papers aforementioned to __________________________________________________ /employee/ of said corporation(s) and authoraized agent thereof ___________________________________________________
        </p>
        ...
        <div className="flex flex-row justify-start items-start gap-x-4">
            ...
            <div>
                ...
                mailed the copy of the same post paid by first-class mail properly addressed to receipient at &nbsp;&nbsp;<span className="font-semibold"> {item?.recipientTitle ? item?.recipientTitle : "--N/A--"}</span>&nbsp;&nbsp; on &nbsp;&nbsp;<span className="font-semibold"> {item?.dateOfMailing ? item?.dateOfMailing : "--N/A--"}</span>&nbsp;&nbsp; 
                ...
                <p>Approx, Description: Sex:<span className="font-semibold">{item?.sex ? item?.sex : "--N/A--"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Skin Color:<span className="font-semibold">{item?.skinColor ? item?.skinColor : "--N/A--"}</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    ...
                    other identifying features/comments:<span className="font-semibold">{item?.otherFeature ? item?.otherFeature : "--N/A--"}</span>
                </p>
            </div>
        </div>
