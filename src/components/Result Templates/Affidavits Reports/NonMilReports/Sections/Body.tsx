...
const Body: React.FC<bodyProps> = (item) => {
  return <div className="flex flex-col items-start gap-y-2 text-sm">
    <p>I &nbsp;<span className="font-semibold">{item?.serverName ? item?.serverName : "__________________"}</span>&nbsp;,beign duly swom, deposes and says:</p>
    ...
    <p className="font-bold">ON &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{item?.dateOfService ? formatDate(item?.dateOfService) : "__________________"} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      {/* AT SOME_TIME_WILL_BE_THERE_FROM_BACKEND_API */}
      , I went to:&nbsp;<span className="font-semibold">{item?.address ? item?.address : "__________________"}</span>&nbsp;</p>
    ...
    <p>And had a conservation with &nbsp;<span className="font-semibold">{item?.name ? item?.name : "__________________"}</span>&nbsp; at the subject of premises, I confirmed it was. Because I asked him/her if he/she was the tenant and he/she replied yes I asked the person(s)
    ...
    <p>In addition I asked if &nbsp;<span className="font-semibold">{item?.name ? item?.name : "__________________"}</span>&nbsp; was in the military service of the United State or the state of New York
    ...
    <div className="font-semibold flex items-center justify-between w-full">
      <div>
        ...
        <p>{item?.mailingAddressDate ? formatDate(item?.mailingAddressDate) : "__________________"}</p>
      </div>
      <div>
        <p><span className="font-semibold">{item?.serverName ? item?.serverName : "__________________"}</span></p>
        <p>Lic#{item?.serverLicense ? item?.serverLicense : "__________________"}</p>
